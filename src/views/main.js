...
class Main extends Component{
  ...
  onVote(choice){
    let questions = this.state.polls;
    ...
    let q = this.state.polls.filter((poll)=> poll._id['$oid'] === questionid);

    if(q.length>=0){
      const questionInd = this.state.polls.indexOf(q[0]);
      let question = q[0];
  ...
  render(){
    const PollDetail = ({ match })=>{
      return <PollDetailPage polls={this.state.polls} pollId={match.params.pollId} onVote={this.onVote}/>
    }
    return(
      <div>
        <Header />
        <Switch>
          <Route path='/home' component={()=> <HomePage polls={this.state.polls} />} />
          <Route exact path='/polls' component={()=> <PollListPage polls={this.state.polls} />} />
